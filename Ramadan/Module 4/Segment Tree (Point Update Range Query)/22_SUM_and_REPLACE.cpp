...
struct SegmentTree {
	...
	void update(vector<int>&a, int l, int r, int id, int tl, int tr) {
		if (tr < l or r < tl) return;
		if (l <= tl and tr <= r) {
			if (t[id].one_two == tr - tl + 1) return;
			if (tl == tr) {
				a[tl - 1] = D[a[tl - 1]];
				t[id].one_two = a[tl - 1] <= 2;
				t[id].res = a[tl - 1];
				return;
			}
		}
		int tm = tl + tr >> 1;
		update(a, l, r, id << 1, tl, tm), update(a, l, r, id << 1 | 1, tm + 1, tr);
		t[id] = combine(t[id << 1], t[id << 1 | 1]);
	}

	void update(vector<int>&a, int l, int r) {
		assert(n > 0);
		update(a, l, r, 1, 1, n);
	}
} seg;
